...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        slope = -beta1 / beta2 if beta2 != 0 else np.inf # Calculate slope of decision boundary
        intercept = -beta0 / beta2 if beta2 != 0 else np.inf # Calculate intercept of decision boundary
        ...
        log_loss = -np.mean(y * np.log(probabilities[:, 1] + 1e-15) + (1 - y) * np.log(1 - probabilities[:, 1] + 1e-15))
        ...
        plt.plot(x_values, decision_boundary, color="black", label="Decision Boundaries:")
